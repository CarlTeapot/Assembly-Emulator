...
public class AssemblyLoad implements EmulatorConstants{
    ...
    static void load(ArrayList<String> tokens, int[] registers, byte[] stack,
        ...
        switch (load) {
            case "li" -> loadImmediate(tokens,registers, stack, stackPointer, index);
            case "lh" -> loadHalf(tokens, registers, stack, stackPointer, index);
            case "lb" -> loadByte(tokens, registers, stack, stackPointer, index);
            case "lw" -> loadWord(tokens, registers, stack, stackPointer, index);
        }
    }
    private static void loadImmediate(ArrayList<String> tokens, int[] registers, byte[] stack, int stackPointer, int index) throws Exception {
        if(!tokens.get(2).chars().allMatch(Character::isDigit)) {
    ...
    private static void loadHalf(ArrayList<String> tokens,  int[] registers, byte[] stack, int stackPointer, int index) throws Exception {
    ...
    private static void loadByte(ArrayList<String> tokens,  int[] registers, byte[] stack, int stackPointer, int index) throws Exception {
        Result result = CPU_RAMPatternMatching.getResult(tokens, registers, stack, stackPointer, index);
        int value = generateValue(stack, result.pointer(), 1);
        registers[result.register()] = value;
    }
    private static void loadWord(ArrayList<String> tokens,  int[] registers, byte[] stack, int stackPointer, int index) throws Exception {
        Result result = CPU_RAMPatternMatching.getResult(tokens, registers, stack, stackPointer, index);
        int value = generateValue(stack, result.pointer(), 4);
        registers[result.register()] = value;
    }

    private static int generateValue(byte[] stack, int pointer, int size) {
        int value = 0;
