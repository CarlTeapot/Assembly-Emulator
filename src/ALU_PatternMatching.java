...
public class ALU_PatternMatching implements EmulatorConstants {
    ...
    static void checkInstruction(ArrayList<String> tokens, int index) throws Exception {
        ...
        int n = tokens.size();
        if (Arrays.asList(immediate).contains(tokens.getFirst())) {
            n--;
        }
        ...
        if (Arrays.asList(immediate).contains(tokens.getFirst()) && !tokens.get(3).matches("-?\\d+")) {
